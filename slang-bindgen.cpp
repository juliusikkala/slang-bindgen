...
static struct {
    ...
    std::vector<std::regex> ignoredMacros;
    std::map<std::string, std::string> translateTypes;
    std::string enumFallbackPrefix = "e";
    bool useByteBool = false;
    bool explicitExportOnly = false;
} options;
...
--ignore-macro <sym>      ignores the specified macro, as if it was defined as
                          empty.
--translate-type <c-name>=<slang-name> skips bindings for the given type and
                                       translates it to the given Slang type.
--explicit-export-only    only includes declarations with the
                          [[clang::annotate("slang-export")]] attribute in C++
...
bool parseOptions(int argc, const char** argv)
{
    ...
    while (i < argc)
    {
        ...
        if (!endFlags && *arg == '-')
        {
            ...
            else if (strcmp(arg, "ignore-macro") == 0)
            {
                options.ignoredMacros.push_back(std::regex(value));
                i++;
            }
            else if (strcmp(arg, "translate-type") == 0)
            {
                std::string from;
                std::string to;
                bool hitEquals = false;
                for (int j = 0; char c = value[j]; ++j)
                {
                    if (c == '=')
                    {
                        hitEquals = true;
                ...
                options.translateTypes[from] = to;
                i++;
            }
            else if (strcmp(arg, "explicit-export-only") == 0)
            {
                options.explicitExportOnly = true;
            }
            else
            {
...
struct ClangSession
{
    ...
    ClangSession()
    {
        ...
        // Clang :/
        argsToClang.push_back("-S"); argsToClang.push_back("dummy.cpp");
        ...
        searchOpts.UseStandardCXXIncludes = true;
        //searchOpts.Verbose = true;

        clang->createDiagnostics(*fs, diagOpts);
    ...
    void determineIntegerSizes(BindingContext& ctx)
    {
        ...
        clang::InputKind inputKind(clang::Language::CXX, clang::InputKind::Format::Source);
...
{
    auto& astCtx = ctx.session->getASTContext();
    clang::QualType qualType = astCtx.getCanonicalTypeDeclType(decl);
    size_t size = astCtx.getTypeSizeInChars(qualType).getQuantity();
    size_t alignment = astCtx.getTypeAlignInChars(qualType).getQuantity();
...
void dumpDecl(BindingContext& ctx, clang::Decl* decl, bool topLevel)
{
    if (options.explicitExportOnly)
    {
        ...
        for (clang::Attr* attr: decl->attrs())
        {
            ...
            {
                auto annotation = clang::cast<clang::AnnotateAttr>(attr);
                if (annotation->getAnnotation() == "slang-export")
                {
                    included = true;
                    break;
                }
...
bool isIgnoredMacro(const std::string& macroName)
{
    for (auto& expr: options.ignoredMacros)
    {
        if (std::regex_match(macroName, expr))
            return true;
    }
    return false;
}
        ...
        case clang::tok::TokenKind::identifier:
            {
                ...
                // Let's pretend this never existed. Goodbye.
                if (isIgnoredMacro(id))
                    continue;
                if (ctx->knownDefinitions.count(id) && !previousWasValue)
                {
...
int main(int argc, const char** argv)
{
    ...
    ctx.output("// Auto-generated bindings by slang-bindgen, from headers:");
